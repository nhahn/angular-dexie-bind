...
(function () {
          ...
          self.$scope.$apply(function() {
            if(change.type == 1) {
              for(var i = 0; i < self.length; i++) {
                if (change.obj[self.context.table.schema.primKey.name] == self[i][self.context.table.schema.primKey.name])
                  return;
              }
                  ...
                  for (var i = 0; i < self.length; i++) {
                    if (self.context.dir == "prev") {
                      if (self[i][self.context.index] <= change.obj[self.context.index]) {
                        self.splice(i, 0, new DexieRecord(self.context, self.$scope, change.obj));
                        return;
                      }
                    } else {
                      if (self[i][self.context.index] >= change.obj[self.context.index]) {
              ...
              for(var i = 0; i < self.length; i++) {
                if(self[i][self.context.table.schema.primKey.name] == change.key) {
                  if(self.$filter(change)) {
                    self[i] = new DexieRecord(self.context, self.$scope, change.obj);
                  }
                  return;
                }
              }
  ...
  window.DexieRecord = (function() {
    ...
    DexieRecord.prototype = {
      //Creates a clean copy of this record, void of angular variables
      $copy: function() {
        var clone = {}
      ...
      //Saves this record, void of angular variables
      $save: function() {
